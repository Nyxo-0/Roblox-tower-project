...
updateScreen.OnClientEvent:Connect(function(to: string, pattern: any, pattern2: any?, pattern3: any?)
	...
	if pattern3 and not pattern3:IsA("ModuleScript") then return end

	wipeScreen()

	local array

	if not pattern3 and not pattern2 then
		array = require(pattern)
	elseif not pattern3 and pattern2 then
		array = append(require(pattern), require(pattern2))
	else
		array = append(append(require(pattern), require(pattern2)), require(pattern3))
	end
